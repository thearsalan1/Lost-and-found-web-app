...
import { LoginTemplate, ClaimStatusTemplate } from "../template/EmailTemplate";
...
export async function requestOtpForNewUser(
  user: IUserDocument,
  email: string,
): Promise<void> {
  await sendOtpToUser(user, email);
...
export async function sendEmailClaim(
  userName: string,
  itemName: string,
  ...
  await sendLoginOtp(
    user.email,
    "Your claimed status has been upadated",
    ClaimStatusTemplate({ userName, itemName, status, reason }),
  );
}
